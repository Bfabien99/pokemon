...
<section class="bg-yellow-200 max-w-screen-md mx-auto shadow-xl rounded-lg">
    ...
    @else
        <div class="flex flex-col justify-center items-center p-4">
            <a href="{{route('pokemon.list')}}#{{$pokemon['slug']}}"
                class="border border-yellow-100 p-2 rounded-sm hover:bg-yellow-300 hover:border-yellow-300">Pokémons</a>
            <div href="{{route('pokemon.detail', $pokemon['slug'])}}" class="p-2 text-center rounded-sm">
                <div class="flex flex-wrap mx-auto items-center gap-2 justify-center">
                    @if (is_array($pokemon['apiPreEvolution']) && count($pokemon['apiPreEvolution']))
                        <a class="border border-blue-400 p-1 hover:bg-blue-400 hover:text-white"
                            href="{{route('pokemon.detail', $pokemon["apiPreEvolution"]['name'])}}">Regresser</a>
                    @else
                            @auth
                                    @if (
                                            !empty(array_filter(auth()->user()->pokemons->toArray(), function ($data) use ($pokemon) {
                                                if ($data['pokemon_id'] == $pokemon['id'])
                                                    return $data;
                                            }))
                                        )
                                            <a class="border border-yellow-400 p-1 hover:bg-yellow-400 hover:text-white"
                                                href="{{route('pokemon.free', $pokemon['id'])}}">Relacher</a>
                                    @else
                                        <a class="border border-orange-400 p-1 hover:bg-orange-400 hover:text-white"
                                            href="{{route('pokemon.catch', $pokemon['id'])}}">Capturer</a>
                                    @endif
                            @endauth
                    ...
                    @if (is_array($pokemon['apiEvolutions']) && count($pokemon['apiEvolutions']))
                        <a class="border border-green-400 p-1 hover:bg-green-400 hover:text-white"
                            href="{{route('pokemon.detail', $pokemon['apiEvolutions'][0]["name"])}}">Evoluer</a>
                    @endif
                </div>
                ...
                <div class="flex flex-col p-2 text-center">
                    ...
                    <div class="bg-gray-600 h-2.5 rounded-full" style="width: 200px">
                        <div class="bg-blue-600 h-2.5 rounded-full progress-bar"
                            style="width: {{$pokemon['stats']['HP']}}px"></div>
                    </div>
                </div>
                <div class="flex flex-col p-2 text-center">
                    <p class="flex items-center justify-between text-lg"><span
                    ...
                    <div class="bg-gray-600 h-2.5 rounded-full" style="width: 200px">
                        <div class="bg-red-600 h-2.5 rounded-full progress-bar"
                            style="width: {{$pokemon['stats']['attack']}}px"></div>
                    </div>
                </div>
                <div class="flex flex-col p-2 text-center">
                    <p class="flex items-center justify-between text-lg"><span
                    ...
                    <div class="bg-gray-600 h-2.5 rounded-full" style="width: 200px">
                        <div class="bg-green-600 h-2.5 rounded-full" style="width: {{$pokemon['stats']['defense']}}px">
                        </div>
                    </div>
                </div>
            </div>
            ...
            @if ($pokemon['apiTypes'])
                @foreach ($pokemon['apiTypes'] as $type)
                    <div class="text-center">
                        ...
                        <a href="{{route('pokemon.type.detail', $type['name'])}}"
                            class="hover:text-blue-400 font-medium">{{$type['name']}}</a>
                    </div>
                @endforeach
            @endif
        </div>
